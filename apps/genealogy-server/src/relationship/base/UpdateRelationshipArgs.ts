/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RelationshipWhereUniqueInput } from "./RelationshipWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { RelationshipUpdateInput } from "./RelationshipUpdateInput";

@ArgsType()
class UpdateRelationshipArgs {
  @ApiProperty({
    required: true,
    type: () => RelationshipWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => RelationshipWhereUniqueInput)
  @Field(() => RelationshipWhereUniqueInput, { nullable: false })
  where!: RelationshipWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => RelationshipUpdateInput,
  })
  @ValidateNested()
  @Type(() => RelationshipUpdateInput)
  @Field(() => RelationshipUpdateInput, { nullable: false })
  data!: RelationshipUpdateInput;
}

export { UpdateRelationshipArgs as UpdateRelationshipArgs };
